...
const schema = Joi.object().keys({
  ...
  raddress: Joi.string()
    ...
    .error(() => {
      return {
        message:
          'Please Enter Valid Address & It Must Be Minimum 10 Characters Long',
      };
    })
    .required()
    .trim(true),
...
accepted_coins:[]
  ...
  wsraddress = async raddress => {
    if (raddress.length <= 25) {
      if (raddress[0] == ' ') {
        showToast('error', "Don't Enter space before raddress.");
  ...
  _onPressSubmit = async () => {
    ...
    const max = selectcoin?.withdraw
      ? selectcoin?.withdraw?.withdraw_max
      : 1000;
    Keyboard.dismiss();
    let val = '';
    ...
    if (!!validata) {
      ...
      showToast('error', 'Minimum withdrawal Amount Is ' + min);
      this.loadingButton.showLoading(false);
    } else if (parseFloat(amount) > parseFloat(max)) {
      showToast('error', 'Maximum withdrawal Amount Is ' + max);
      this.loadingButton.showLoading(false);
    } else {
      try {
        const obj = {
  ...
  onsubmit = async () => {
    ...
    if (!!validata) {
      ...
      try {
        ...
        const res = await backEndCallObj('/payments/withdraw', obj);
        if (res?.success) {
           ...
           labelStyle={{color:"#a9efdb"}}
            ...
            <View style={styles.sidecont}>
              <TextInput
                ...
                placeholder={`Enter ${this.state.coinname} Receiver Address`}
                maxLength={25}
                placeholderTextColor={'#79c1ac'}
                returnKeyType="done"
            ...
            <View style={{flexDirection: 'row', alignSelf: 'center'}}>
              ...
              <Text
                ...
                <Text style={{fontFamily: 'Montserrat-ExtraBold'}}>
                  {this?.state?.selectcoin?.withdraw
                    ? this?.state?.selectcoin?.withdraw?.withdraw_max
                    : '50'}
                  ,
                </Text>{' '}
                Max -{' '}
