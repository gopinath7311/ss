...
import get_transactnhis from '../../../../../redux/actions/gettranshistoryAction';
import LinearGradient from 'react-native-linear-gradient';
import IndeTrans from './indextransac';
...
class Transactionhisty extends Component {
  state = {
    refreshing: false,
...
async componentDidMount(){
  ...
  render() {
    const transhis = this?.props?.gettranshis;
    const {modaldata} = this.state;
    return (
      <View style={{backgroundColor:"#000000",flex:1}}>
        <IndeTrans props={this.props} />
        </View>
    );
  }
}
...
const mapStateToProps = state => {
  return {
    gettranshis: state.gettranshis,
  };
};

export default connect(mapStateToProps)(Transactionhisty);
